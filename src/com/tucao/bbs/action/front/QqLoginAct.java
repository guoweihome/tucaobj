...
public class QqLoginAct {
	...
	public static final String TPL_QQBIAN = "tpl.qqbind";
	
	@RequestMapping(value = "/qqindex.jspx")
	...
	@RequestMapping(value = "/afterLogin.jspx")
	public String afterLogin(HttpServletRequest request, ModelMap model,HttpServletResponse response) throws IOException {//throws IOException
...
//            我们的网站被CSRF攻击了或者用户取消了授权, 做一些数据统计工作          
             ...
             tokenExpireIn = accessTokenObj.getExpireIn();
         	 ...
         	 session.setAttribute(request, response, "qq_access_token",accessToken);
         	 session.setAttribute(request, response, "qq_token_expirein",String.valueOf(tokenExpireIn));             

             // 利用获取到的accessToken 去获取当前用的openid -------- start
             OpenID openIDObj =  new OpenID(accessToken);
             openID = openIDObj.getUserOpenID();
             
             session.setAttribute(request, response, "qq_openid",openID); //openID
             
             UnifiedUser undifiedUser =unifiedUserMng.getUnifiedUserByOpenId(openID,true);//openID
             //查询此用户是否已经绑定了,未绑定跳转到绑定页面
             if(undifiedUser==null){
         		 FrontUtils.frontData(request, model, site);
         		 model.put("qqlogin", "1");//qq登录是1要判断是否和原来账户绑定
...
// 			model.put("content", this.getContent(site));
// 			
// 			//活跃大神
// 			List<BbsUser> bbsUserList=bbsUserMng.getHotMember(8);
// 			model.put("bbsUserList", bbsUserList);
// 			
// 			//热门标签
// 			List<BbsForum> bbsfromList= bbsForumMng.getList(site.getId());
// 			model.put("bbsfromList", bbsfromList);
...
//			        sb.append("<a class='S_func2' href='javascript:void(0);'>收藏</a><em class='S_txt3'>|</em>");
		            ...
		            sb.append("</script>");
		            //sb.append("<em class='S_txt3'>|</em><span id='qq_weishare_"+bbs.getId()+"'><a class='S_func2' href='javascript:vod(0)' onclick='javascript:qq_wei("+bbs.getId()+")' ><img src='/bj/img/share_icon_qq.png'/></a></span>");
	 ...
	 */
	@RequestMapping(value = "/qquser_bind.jspx")
	public String qquser_bind(HttpServletRequest request,
		...
		return FrontUtils.getTplPath(request, site.getSolutionPath(),TPLDIR_INCLUDE, TPL_QQBIAN);
	}
	 ...
	 * qq与账号绑定
	 */
	@RequestMapping(value = "/user_loginbind.jspx")
	public void user_loginbind(String username, String password, String captcha,
		...
		System.out.println("获取openid:"+(String)session.getAttribute(request, "qq_openid"));
		Authentication auth = authMng.login(username, password, ip,(String)session.getAttribute(request, "qq_openid"),true,request, response, session);
	 ...
	 */
	@RequestMapping(value = "/user_loginnobind.jspx")
	public void user_loginnobind(BbsUserExt userExt,String processUrl,  String returnUrl,HttpServletRequest request,HttpServletResponse response,ModelMap model) {
		AjaxResponseMessage ajaxmessage = new AjaxResponseMessage();
		CmsSite site = CmsUtils.getSite(request);
		try {
		String ip = RequestUtils.getIpAddr(request);
		...
		user = bbsUserMng.registerMember(username, "", "000000", ip, groupId, userExt,(String)session.getAttribute(request, "qq_openid"),null);
		...
		ajaxmessage.setMsg(returnUrl);
			...
			ajaxmessage.setMsg("qq不与账号绑定逻辑出错!");
			ajaxmessage.setSuccess(false);
