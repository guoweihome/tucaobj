...
public class BbsTopicDaoImpl extends HibernateBaseDao<BbsTopic, Integer>
	...
	public List<BbsTopic> getListTopShow(Integer page){
		...
		String hql = "from BbsTopic bean where bean.status>= "+String.valueOf(BbsTopic.NORMAL) +" and bean.hasTopShow='1' order by bean.createTime desc ";
		return getSession().createQuery(hql).setFirstResult((page-1)*10).setMaxResults(20).list();
	}
	...
	public List<BbsTopic> getNewList(Integer count,String datetime) {
		String hql = "from BbsTopic bean where bean.createTime> str_to_date('"+datetime+"','%Y-%m-%d %H:%i:%s')"+" order by bean.createTime desc";
		return getSession().createQuery(hql).setFirstResult(0).setMaxResults(count).setCacheable(false).list();
	}
	
	@SuppressWarnings("unchecked")
	public List<BbsTopic> getShuoshList(Integer count){
		String hql = "from BbsTopic bean where bean.status>= "+String.valueOf(BbsTopic.NORMAL) +" order by bean.createTime desc";
