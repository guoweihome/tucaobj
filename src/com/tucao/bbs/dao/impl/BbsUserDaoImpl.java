...
public class BbsUserDaoImpl extends HibernateBaseDao<BbsUser, Integer>
	...
	public List<BbsUser> getHotMember(Integer count){
		Finder f = Finder.create("select bean from BbsUser bean where bean.disabled=false order by bean.point desc ");
		Query query = f.createQuery(getSession());
		query.setMaxResults(count);
		return query.list();
	}
	...
	public int getLongUserId(String sequchar){
		String hql = "select _nextval('"+sequchar+"')";
		
		//System.out.println("测试："+this.getSession().createSQLQuery(hql).uniqueResult());
		int num=Integer.parseInt(this.getSession().createSQLQuery(hql).uniqueResult().toString());
